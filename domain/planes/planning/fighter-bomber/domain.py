...
loadTime = 1.0
baseFighterAccel = 1.0
baseBomberAccel = 0.5
baseFighterDecel = 0.5
baseBomberDecel = 0.25
...
class City:
	...
	def __init__(self, name, loc, production, airdef):
		self.name = name
		self.loc = loc
		self.fighters = []
		self.bombers = []
	...
	def land_plane(self, plane):
		if plane.type == "bomber":
			self.bombers.append(plane)
		elif plane.type == "fighter":
			self.fighters.append(plane)
	
	def launch_plane(self, plane):
		if plane.type == "bomber":
			self.bombers.append(plane)
		elif plane.type == "fighter":
			self.fighters.append(plane)
